...
if [ -f "design/gdd/game-concept.md" ] || [ -f "doc/game-concept.md" ]; then
  FRESH_PROJECT=false
fi
...
for src_dir in src app lib; do
  if [ -d "$src_dir" ]; then
    SRC_CHECK=$(find "$src_dir" -type f \( -name "*.gd" -o -name "*.cs" -o -name "*.cpp" -o -name "*.c" -o -name "*.h" -o -name "*.hpp" -o -name "*.rs" -o -name "*.py" -o -name "*.js" -o -name "*.ts" -o -name "*.tsx" \) 2>/dev/null | head -1)
    if [ -n "$SRC_CHECK" ]; then
      FRESH_PROJECT=false
      break
    fi
...
for src_dir in src app lib; do
  if [ -d "$src_dir" ]; then
    COUNT=$(find "$src_dir" -type f \( -name "*.gd" -o -name "*.cs" -o -name "*.cpp" -o -name "*.c" -o -name "*.h" -o -name "*.hpp" -o -name "*.rs" -o -name "*.py" -o -name "*.js" -o -name "*.ts" -o -name "*.tsx" \) 2>/dev/null | wc -l)
    SRC_FILES=$((SRC_FILES + COUNT))
  fi
done

DESIGN_FILES=0
for gdd_dir in "design/gdd" "doc"; do
  if [ -d "$gdd_dir" ]; then
    COUNT=$(find "$gdd_dir" -type f -name "*.md" 2>/dev/null | wc -l)
    DESIGN_FILES=$((DESIGN_FILES + COUNT))
  fi
done
...
if [ "$SRC_FILES" -gt 50 ] && [ "$DESIGN_FILES" -lt 5 ]; then
  ...
  echo "    Suggested action: /reverse-document design app/[system] or lib/game/[system]"
...
if [ -d "lib" ] || [ -d "src/core" ] || [ -d "src/engine" ]; then
  if [ ! -d "docs/architecture" ]; then
    echo "⚠️  GAP: Core systems exist but no docs/architecture/ directory"
    echo "    Suggested action: Create docs/architecture/ and run /architecture-decision"
  else
    ADR_COUNT=$(find docs/architecture -type f -name "adr-*.md" 2>/dev/null | wc -l)
    ADR_COUNT=$(echo "$ADR_COUNT" | tr -d ' ')

    if [ "$ADR_COUNT" -lt 3 ]; then
      echo "⚠️  GAP: Core systems exist but only $ADR_COUNT ADR(s) documented"
      echo "    Suggested action: /architecture-decision for each major technical choice"
    fi
  fi
fi

# --- Check 4: Gameplay systems without design docs ---
for gameplay_dir in "lib/game" "src/gameplay"; do
    ...
    if [ -n "$GAMEPLAY_SYSTEMS" ]; then
      while IFS= read -r system_dir; do
        ...
        if [ "$file_count" -ge 5 ]; then
          ...
          design_doc_3="doc/${system_name}-system.md"
          design_doc_4="doc/${system_name}.md"

          if [ ! -f "$design_doc_1" ] && [ ! -f "$design_doc_2" ] && [ ! -f "$design_doc_3" ] && [ ! -f "$design_doc_4" ]; then
            echo "⚠️  GAP: Gameplay system '$gameplay_dir/$system_name/' ($file_count files) has no design doc"
            echo "    Expected: design/gdd/${system_name}-system.md or doc/${system_name}.md"
            echo "    Suggested action: /reverse-document design $gameplay_dir/$system_name"
          fi
        fi
