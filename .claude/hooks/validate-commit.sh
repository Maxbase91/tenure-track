...
DESIGN_FILES=$(echo "$STAGED" | grep -E '^(design/gdd|doc)/' | grep '\.md$')
...
DATA_FILES=$(echo "$STAGED" | grep -E '^(public/data|lib)/.*\.json$')
...
CODE_FILES=$(echo "$STAGED" | grep -E '^lib/game/')
if [ -n "$CODE_FILES" ]; then
    while IFS= read -r file; do
        if [ -f "$file" ]; then
            if grep -nE '(damage|health|speed|rate|chance|cost|duration)[[:space:]]*[:=][[:space:]]*[0-9]+' "$file" 2>/dev/null; then
                WARNINGS="$WARNINGS\nCODE: $file may contain hardcoded gameplay values. Use constants file."
            fi
        fi
    done <<< "$CODE_FILES"
fi
...
SRC_FILES=$(echo "$STAGED" | grep -E '^(app|lib)/')
if [ -n "$SRC_FILES" ]; then
    while IFS= read -r file; do
        if [ -f "$file" ]; then
